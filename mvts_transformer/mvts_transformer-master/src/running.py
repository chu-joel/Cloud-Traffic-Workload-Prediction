...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    eval_start_time = time.time()
    with torch.no_grad():
        aggr_metrics, per_batch, __,__ = val_evaluator.evaluate(epoch, keep_all=True)
    eval_runtime = time.time() - eval_start_time
...
class UnsupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        predicted= flatten_comprehension(per_batch['predictions'])
        truth=flatten_comprehension(per_batch['targets'])
        if keep_all:
            return self.epoch_metrics, per_batch, list(itertools.chain(*predicted)), list(itertools.chain(*truth))
        else:
            return self.epoch_metrics
...
class SupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            # Zero gradients, perform a backward pass, and update the weights.
            self.optimizer.zero_grad()
            total_loss=total_loss.type(torch.float32)
    ...
    def evaluate(self, epoch_num=None, keep_all=True, mode="Val"):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            # regression: (batch_size, num_labels); classification: (batch_size, num_classes) of logits
            predictions = self.model(X.to(self.device), padding_masks)
            # print(np.c_[predictions.cpu().detach().numpy(), targets.cpu().detach().numpy()])
            # (batch_size,) loss for each sample in the batch
            loss = self.loss_module(predictions, targets)
            batch_loss = torch.sum(loss).cpu().item()
